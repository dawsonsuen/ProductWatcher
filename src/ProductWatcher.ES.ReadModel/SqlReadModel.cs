...
namespace ProductWatcher.ES.ReadModel
{
    public class SqlReadModel : IReadFromReadModel, IWriteReadModel
    {
        ...
        {
            var readModel = new ReadModelSql();

            using (var cmd = _db.CreateCommand())
            {
                cmd.CommandText = "select id, stream_id, type, last_update, body from read_model where stream_id=@streamId";
                CreateParam(cmd, "@streamId", DbType.Guid, id);
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        readModel.Id = reader.GetInt64(0);
